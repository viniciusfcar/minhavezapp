...
export default class EditarUsernameScreen extends React.Component {
    ...
    alterar = async () => {
        ...
        form.append('new_username', this.state.new_username);
        form.append('token', await AsyncStorage.getItem('@MinhaVezSistema:token'));

        const apiCall = await fetch('http://165.22.185.36/api/usuario/editar_username/', {
            method: 'POST',
            body: form,
        });

        if(apiCall.status = '404') {
            Alert.alert('Atenção!', 'Você está tentando alterar o username de outro usuário ou que não existe.');
