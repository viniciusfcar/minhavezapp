...
export default class DetalhesExameScreen extends React.Component {
  ...
  onUnidade = async () => {
    ...
    const apiCall = await fetch('http://165.22.185.36/api/unidade_saude/'+
                    this.props.navigation.getParam('exame').id + '/exame_unidade/')
  ...
  onFilas = async () => {
    ...
    var lista = this.props.navigation.getParam('exame').filas;
    var newLista = [];
    var fichas = [];
  ...
  marcar = async (pref) => {
    const form = new FormData();
    form.append("id_exame", this.props.navigation.getParam('exame').id);
    form.append("token", await AsyncStorage.getItem('@MinhaVezSistema:token'));
    form.append("preferencial", pref);

    const apiCall = await fetch('http://165.22.185.36/api/ficha/cadastro_ficha_exame/', {
      method: 'POST',
      body: form,
    });
  ...
  verifica(){
    if(this.props.navigation.getParam('exame').create_fila){ 
          ...
          <Card>
            <CardItem header>
              <Left>
                  ...
                  <Text>{this.props.navigation.getParam('exame').nome}</Text>
              </Left>
            </CardItem>
            <CardItem header>
              <Left>
                  <Text style={{fontWeight: 'bold', color: '#4169E1'}}>Hora: </Text>
                  <Text>{this.props.navigation.getParam('exame').hora}</Text>
              </Left>
              <Right>
                  <Text style={{fontWeight: 'bold', color: '#4169E1'}}>Data: </Text>
                  <Text>{this.props.navigation.getParam('exame').data}</Text>
              </Right>
            </CardItem>
            <CardItem header bordered>
              <Left>
                <Text style={{fontWeight: 'bold', color: '#4169E1'}}>Status: </Text>
                  <Text>{this.props.navigation.getParam('exame').status}</Text>
              </Left>
              <Right>
                <Text style={{fontWeight: 'bold', color: '#4169E1'}}>Tipo: </Text>
                <Text>{this.props.navigation.getParam('exame').tipo}</Text>
              </Right>
            </CardItem>
            <CardItem header>
              <Left>
